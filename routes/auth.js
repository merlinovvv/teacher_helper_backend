...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (existingUserName) {
      return res
        .status(400)
        .json(getErrorResponse("Користувач з таким ніком вже існує"));
    }
    ...
    if (existingUserEmail) {
      return res
        .status(400)
        .json(getErrorResponse("Користувач з таким email вже існує"));
    }

    const user = new User({ username, email, password });
    await user.save();
    const token = jwt.sign({ id: user._id, user_role: user.user_role }, process.env.JWT_SECRET, {
      expiresIn: "1h",
    });
    res
      .status(201)
      .json(getSuccessResponse({ token }, "Ви успішно зареєструвались"));
  } catch (err) {
    console.log(err);
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json(getErrorResponse(`Користувач з ніком ${username} не знайдений`));
    }
    ...
    const emails = process.env.ADMIN_EMAIL.split(',')
    ...
    return res
      .status(200)
      .json(getSuccessResponse({ accessToken, refreshToken }));
  } catch (err) {
    console.error(err);
    return res
      .status(500)
      .json(
        getErrorResponse("Помилка авторизації. Зверніться до адміністратора")
      );
  }
});
...
router.post("/refresh", async (req, res) => {
  ...
  if (!refreshToken) {
    return res
      .status(400)
      .json(getErrorResponse("Час сесії вичерпано. Увійдіть в акаунт знову"));
  }
  ...
  try {
    ...
    return res
      .status(201)
      .json(getSuccessResponse({ accessToken, refreshToken: newRefreshToken }));
  } catch (err) {
    if (err.name === "TokenExpiredError") {
      return res
        .status(401)
        .json(
          getErrorResponse("Час сесії вичерпано. Увійдіть в акаунт знову", 401)
        );
    }
...
router.post("/me", authenticate, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(getSuccessResponse({ user }, "Налаштування користувача збережено"));
  } catch (err) {
...
router.get("/check-payment", authenticate, async (req, res) => {
  try {
    ...
    const match = allDonates.find(
      (item) => item.clientName === user.username && Number(item.amount) >= 99
    );
    ...
    return res
      .status(404)
      .json(getErrorResponse("Дані про оплату не знайдені"));
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json(getErrorResponse("Помикла отримання даних з Donatello"));
  }
});
