            ...
            report = normalReport.map(student => {
                ...
                return {
                    ...
                    groups: groups.map(group => {
                        ...
                        const assigments = relevantRatings.filter(({date}) => group.assigment_dates.some((b_date) => new Date(b_date).getTime() === new Date(date).getTime()));
                        const controls = relevantRatings.filter(({date}) => group.control_dates.some((b_date) => new Date(b_date).getTime() === new Date(date).getTime()));
