...
router.post("/subjects", authenticate, async (req, res) => {
  ...
  try {
    res
      .status(200)
      .json(getSuccessResponse({}, `Предмет ${name} успішно додано!`));
  } catch (err) {
  ...
  try {
    ...
    res
      ...
      .json(
        getSuccessResponse(
          {},
          `Предмет "${existingSubject?.name}" успішно видалений!`
        )
      );
  } catch (err) {
...
router.post("/classes", authenticate, async (req, res) => {
  ...
  try {
    ...
    if (existingClass) {
      ...
      res
        .status(200)
        .json(getSuccessResponse({}, `${name} успішно оновлений!`));
    } else {
...
router.delete("/classes", authenticate, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json(
        getSuccessResponse({}, `${existingClass?.name} успішно видалений!`)
      );
  } catch (err) {
...
router.post("/groups", authenticate, async (req, res) => {
  ...
  try {
    ...
    const existingLinks = await Groups.find(
      { userId: req.user.id },
      "subject schoolClass"
    ).lean();
    ...
    for (const groupData of groupsArray) {
      ...
      if (!subjectExists) {
        return res
          .status(404)
          .json(getErrorResponse(`Предмет не знайдений: ${subject}`));
      }

      if (!classesExists) {
        return res
          .status(404)
          .json(getErrorResponse(`Клас не знайдений: ${schoolClass}`));
      }
    ...
    res
      .status(200)
      .json(getSuccessResponse({ results }, "Групи оцінок успішно оновлені"));
  } catch (err) {
  ...
  async (req, res) => {
    ...
    if (!type_report) {
      res
        .status(400)
        .json(
          getErrorResponse(`Помилка! Встановіть тип звітів в налаштуваннях!`)
        );
    }

    const existingGradesGroupsReports = await GradesGroupsReports.findOne({
      subject,
      schoolClass,
      userId,
      reportType: type_report,
    });
    ...
    if (existingGradesGroupsReports) {
      ...
      const gradesGroupsReports = new GradesGroupsReports({
        subject,
        schoolClass,
        userId,
        reportType: type_report,
...
router.get("/report", authenticate, async (req, res) => {
  ...
  if (existingGradesGroupsReports) {
    const { userId, subject, schoolClass, reportType } =
      existingGradesGroupsReports;

    try {
      const filePath = path.join(
        __dirname,
        `../uploads/${userId}_${subject}_${schoolClass}_${reportType}.xlsx`
      );
      ...
      // Функция для вычисления статистики по уровням
      function calculateStatisticsByGroups(data) {
        const groupStatistics = {};
        const totalStatistics = {
          start: { count: 0, percent: 0 },
          avg: { count: 0, percent: 0 },
          sufficient: { count: 0, percent: 0 },
        ...
        data.forEach((student) => {
          student.groups.forEach((group) => {
            ...
            const avg = Number(group.groupAvg); // Преобразуем groupAvg в число

            if (!groupStatistics[groupName]) {
            ...
            if (!isNaN(avg)) {
              groupStatistics[groupName].total++;
              groupStatistics[groupName].avgGroup += avg;
              if (avg >= 1 && avg <= 3) {
                groupStatistics[groupName].start.count++;
              } else if (avg >= 4 && avg <= 6) {
                groupStatistics[groupName].avg.count++;
              } else if (avg >= 7 && avg <= 9) {
                groupStatistics[groupName].sufficient.count++;
              } else if (avg >= 10 && avg <= 12) {
                groupStatistics[groupName].high.count++;
              }
            }
          });

          // Обработка avg_groups для общей статистики
          const studentAvg = Number(student.avg_groups);
          totalStatistics.avgGroup += studentAvg;
          if (!isNaN(studentAvg)) {
            totalStatistics.total++;
            if (studentAvg >= 1 && studentAvg <= 3) {
              totalStatistics.start.count++;
            } else if (studentAvg >= 4 && studentAvg <= 6) {
              totalStatistics.avg.count++;
            } else if (studentAvg >= 7 && studentAvg <= 9) {
              totalStatistics.sufficient.count++;
            } else if (studentAvg >= 10 && studentAvg <= 12) {
              totalStatistics.high.count++;
            }
          }
        });

        // Рассчитать проценты для каждой группы
        for (const group in groupStatistics) {
          const stats = groupStatistics[group];
          for (const level in stats) {
            if (level !== "total" && level !== "avgGroup") {
              stats[level].percent = (
                (stats[level].count / stats.total) *
                100
              ).toFixed(2);
            }
          }
          stats.avgGroup = (stats.avgGroup / stats.total).toFixed(2);
        }

        // Рассчитать проценты для общей статистики
        for (const level in totalStatistics) {
          if (level !== "total" && level !== "avgGroup") {
            totalStatistics[level].percent = (
              (totalStatistics[level].count / totalStatistics.total) *
              100
            ).toFixed(2);
          }
        }
        totalStatistics.avgGroup = (
          totalStatistics.avgGroup / totalStatistics.total
        ).toFixed(2);
        return { ...groupStatistics, "Всі групи": totalStatistics };
      }
      ...
      switch (reportType) {
        case "human":
          for (
            let listNum = 0;
            listNum < workbook.SheetNames?.length;
            listNum++
          ) {
            ...
            for (let key in file) {
              const [currentColumn, currentRow] =
                key.match(/[A-Za-z]+|[0-9]+/g);
              const rowNumber = Number(currentRow);
              if (
                rowNumber >= 4 &&
                key.includes("A") &&
                Number.isInteger(Number(key[1]))
              ) {
                if (!names[key]) {
                  names[key] = {
                    name: file[key]["v"],
              ...
              if (names[rowKey] && currentColumn !== "A") {
                ...
                const rating = Number.isInteger(Number(file[key]?.["v"]))
                  ? Number(file[key]?.["v"])
                  : "";

                if (!ignoreTypes.includes(type) && rating) {
                  names[rowKey].ratings.push({
                    type: type || "",
                    rating: rating,
                    date: date || "",
                  });
          ...
          const normalReportHuman = Object.values(names).map(
            ({ name, ratings = [] }) => ({
              name,
              ...
              groups: groups.map((group) => {
                ...
                const relevantRatings = student.ratings.filter(({ date }) => {
                  const dateInGroup = (date) => {
                    ...
                    return (
                      group.assigment_dates.some(
                        (gDate) => parsedDate === ukraineDate(gDate)
                      ) ||
                      group.control_dates.some(
                        (gDate) => parsedDate === ukraineDate(gDate)
                      )
                    );
                  };
                ...
                const assigments = relevantRatings.filter(({ date }) => {
                  return group.assigment_dates.some(
                    (b_date) => ukraineDate(b_date) === date
                  );
                });
                ...
                const assigmentsAvg =
                  assigments.reduce(
                    (sum, { rating }) => sum + (Number(rating) || 0),
                    0
                  ) / (assigments.length || 1);
                const controlsAvg =
                  controls.reduce(
                ...
                let groupAvg = controls.length
                  ? Math.round((assigmentsAvg + controlsAvg) / 2)
                  : Math.round(assigmentsAvg);
                ...
                return {
                  ...
                  assigments: [
                    ...assigments,
                    {
                      rating: Math.round(assigmentsAvg),
                      type: "Середня",
                      date: "",
                    },
                  ],
        ...
        case "novi_znannya":
          ...
          const normalReportNovyZnannya = Object.values(names).map(
            ({ name, ratings = [] }) => ({
              name,
              ratings: ratings.map(({ date, rating }) => {
      ...
      res.status(200).json(
        getSuccessResponse({
          groupStats: calculateStatisticsByGroups(report),
          report,
          groups,
      ...
      res
        .status(500)
        .json(getErrorResponse("Сталася помилка при отриманні звіту"));
    }
  } else {
